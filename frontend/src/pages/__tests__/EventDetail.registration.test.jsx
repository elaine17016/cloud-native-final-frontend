...
describe('EventDetail registration flow', () => {
  ...
  it('shows admin eligibility requirements in registration modal', async () => {
    const eligibility = {
      version: 1,
      adult: {
        gender: 'ANY',
        healthUnlimited: false,
        healthNoDiseases: ['Heart disease / cardiovascular condition', 'Hypertension'],
        otherRestrictions: ['Wear comfortable shoes']
      },
      child: null
    };
    eventDetailMocks.getEvent.mockResolvedValue({
      data: {
        id: 'evt-1',
        title: 'Spring Family Day',
        description: `Event description\n<!--CETS_ELIGIBILITY:${JSON.stringify(eligibility)}-->`,
        status: 'PUBLISHED',
        allowed_sites: ['HSINCHU'],
        created_at: '2026-05-01T10:00:00+08:00',
        sessions: [{
    ...
    expect(await screen.findByText('Eligibility requirements')).toBeInTheDocument();
    expect(screen.getByText(/Must confirm no listed conditions/i)).toBeInTheDocument();
